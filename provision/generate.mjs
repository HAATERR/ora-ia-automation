#!/usr/bin/env node
// GENERADOR: compila un flow-spec (flows/<x>.flow.json, derivado del flujograma) a los
// templates de n8n (templates/<flow>.template.json) tokenizados con {{ORA_*}}.
//
//   node provision/generate.mjs flows/primedental.flow.json
//
// Después: provision-client.mjs <client.json> los crea en n8n.
// Generaliza los builders validados (_build-analisis / _build-seguimientos).
...
if (!specPath) { console.error('Uso: node provision/generate.mjs flows/<x>.flow.json'); process.exit(1); }
const spec = JSON.parse(await readFile(specPath, 'utf8'));
console.log(`\n▶ Generando flows de "${spec.project}"\n`);
for (const [flowName, flow] of Object.entries(spec.flows)) {
  const compile = COMPILERS[flowName];
  if (!compile) { console.log(`  [skip] ${flowName} (sin compilador)`); continue; }
  const wf = compile(flow);
  const out = `templates/${flowName}.template.json`;
  await writeFile(out, JSON.stringify(wf, null, 2) + '\n');
  const tokens = [...new Set(JSON.stringify(wf).match(/\{\{ORA_[A-Z0-9_]+\}\}/g) || [])].sort();
  console.log(`  ✅ ${out} — ${wf.nodes.length} nodos | tokens: ${tokens.length}`);
}
console.log('\n✅ Generación completa.\n');
